...
module.exports = async ({ env, browsers, compile }) => {
  ...
  if (browsers.find(b => !/(chrome|safari|extensions)/.test(b))) {
    ...
    console.log('  Please use one of the following: chrome, extensions, safari')
    console.log()
  ...
  for (browser of browsers) {
    const name = `${slugify(manifest.name)}.${browser}extension`
    const version = manifest.version

    await progress(
      async () => {
        await execa('rm', ['-f', `artifacts/${name}-${env}-v${version}.zip`])
        if (browser === 'chrome') {
          process.chdir(`packs`)
          await execa('zip', ['-r', '-D', `../artifacts/${name}-${env}-v${version}.zip`, `${name}`])
          process.chdir(`..`)
        } else {
          process.chdir(`packs/${name}`)
          await execa('zip', ['-r', '-D', `../../artifacts/${name}-${env}-v${version}.zip`, `./`])
          process.chdir(`../..`)
        }
      },
      {
